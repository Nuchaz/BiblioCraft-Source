                    ...
                    if (resultStack != ItemStack.EMPTY) {
                        if (Utils.checkForValidRecipeIngredients(bookGrid, player, false)) {
                            ...
                            if (result != ItemStack.EMPTY) {
                                if (Utils.checkForValidRecipeIngredients(bookGrid, player, true)) // remove valid ingredients from
                                                                                            // inventory
                                {
                                    ...
                                    Utils.sendARecipeBookTextPacket(player, I18n.translateToLocal("gui.recipe.failed"),
                                            inventorySlot);
                                }
                                return null;
                            } else {
                            ...
                            Utils.sendARecipeBookTextPacket(player, I18n.translateToLocal("gui.recipe.missing"), inventorySlot);
                            return null;
                        }
                    } else {
                        Utils.sendARecipeBookTextPacket(player, I18n.translateToLocal("gui.recipe.invalid"), inventorySlot);
                        return null;
                    }
                }
                Utils.sendARecipeBookTextPacket(player, I18n.translateToLocal("gui.recipe.wrong"), inventorySlot);
            } else {
                Utils.sendARecipeBookTextPacket(player, I18n.translateToLocal("gui.recipe.disabled"), inventorySlot);
            }
            return null;
        }
