...
public class FileUtil 
{
	...
	/**
	 * Returns an int from the array of books that can be found in the stored folder.
	 * Uses the parameters author and title to search the names of the saved books after
	...
	/**
	 * Takes an item stack and if it is the correct item, it will convert the item to
	...
	public ItemStack loadBook(World world, ItemStack stack, int bookint)
	{
		...
		if (booksArray != null)
		{
			...
			try 
			{
				...
				// this totally work, it reads the book and goes through the entire thing, line by line.
				String line; 
...
// I am most likely going to have to put in some checks to for line length and skip lines that are too long?, will test, if minecraft auto-skips, thats easier
	...
	/**
	 * Scans the directory of saved books and returns a string array of all the saved books.
	 * 
	 * @param world
	 * @return
	 */
		...
		if (book != ItemStack.EMPTY)
		{
			...
			if (nbt != null)
			{
				...
				if (savedMeta)
				{
					...
					File bookFile = new File(getSaveDir(world), metaname);
					try
					{
